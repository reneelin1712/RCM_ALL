...
def interpret_model():
    ...
    output_dir = f'output_img_{model_name}_featureablation'
    ...
    # Define the forward functions for policy and discriminator networks
    ...
    def forward_func_discriminator(input_data_disc, x_act):
    ...
    for idx, row in df_generated.iterrows():
        ...
        for key, step_idx in indices_to_analyze.items():
            ...
            # Use Feature Ablation for policy network
            fa_policy = FeatureAblation(forward_func_policy)
            attributions_fa_policy = fa_policy.attribute(
                input_data, target=action_taken, feature_mask=None
            )
            attributions_policy_np = attributions_fa_policy.squeeze().cpu().detach().numpy()
            ...
            feature_importance_policy_df.to_csv(
                os.path.join(output_csv_dir, f'trajectory_{idx+1}_{key}_policy_fa_feature_importance.csv'), index=False
            )
            ...
            # Use Feature Ablation for discriminator network
            fa_disc = FeatureAblation(forward_func_discriminator)
            attributions_fa_disc = fa_disc.attribute(
                (input_data_disc, act_one_hot),
                target=None,
                feature_mask=(None, None)
            )
            attributions_input = attributions_fa_disc[0].squeeze().cpu().detach().numpy()
            attributions_action = attributions_fa_disc[1].squeeze().cpu().detach().numpy()
            ...
            feature_importance_disc_df.to_csv(
                os.path.join(output_csv_dir, f'trajectory_{idx+1}_{key}_discriminator_fa_feature_importance.csv'),
                index=False
            )
            ...
            axes[0].set_title('Policy Network Feature Ablation Importance')
            ...
            axes[1].set_title('Discriminator Feature Ablation Importance')
    ...
    for key in aggregated_importance_policy.keys():
        if aggregated_importance_policy[key]:
            ...
            # Save DataFrame to CSV
            feature_importance_policy_df.to_csv(
                os.path.join(output_csv_dir, f'aggregated_{key}_policy_fa_feature_importance.csv'), index=False
            )
            ...
            # Save DataFrame to CSV
            feature_importance_disc_df.to_csv(
                os.path.join(output_csv_dir, f'aggregated_{key}_discriminator_fa_feature_importance.csv'),
                index=False
            )
            ...
            axes[0].set_title('Aggregated Policy Network Feature Ablation Importance')
            axes[0].invert_yaxis()  # Highest importance at the top
            ...
            axes[1].set_title('Aggregated Discriminator Feature Ablation Importance')
            axes[1].invert_yaxis()  # Highest importance at the top
