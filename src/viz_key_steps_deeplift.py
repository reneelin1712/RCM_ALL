...
def interpret_model():
    ...
    output_dir = f'output_img_{model_name}_20241127_deeplift'
    ...
    for idx, row in df_generated.iterrows():
        ...
        for key, step_idx in indices_to_analyze.items():
            ...
            ig_policy = DeepLift(forward_func_policy)
            attributions_ig_policy = ig_policy.attribute(input_data, baselines=input_data * 0,target=action_taken)
            ...
            ig_disc = DeepLift(forward_func_discriminator)
            baseline_input_data_disc = input_data_disc * 0
            baseline_act_one_hot = act_one_hot * 0
            attributions_ig_disc = ig_disc.attribute((input_data_disc, act_one_hot),baselines=(baseline_input_data_disc, baseline_act_one_hot))
